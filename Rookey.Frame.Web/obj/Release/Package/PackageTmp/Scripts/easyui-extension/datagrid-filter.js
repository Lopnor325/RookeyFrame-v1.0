    ...
    function init(target, filters) {
        ...
        function createFilter(frozen) {
            ...
            for (var i = 0; i < fields.length; i++) {
                ...
                try {
                    if (input.hasClass('combotree-f')) { //下拉树
                        input.combotree({
                    ...
                    else if (input.hasClass('combobox-f')) { //下拉列表框
                        input.combobox({
                            onChange: function (newValue, oldValue) {
                                var options = $(this).combobox('options');
                                if (options.valueField == 'ckId' && options.textField == 'ckName') {
                                    addFilterRule(target, {
                                        field: $(this).attr('name'),
                                        op: newValue ? 'equal' : 'nofilter',
                                        value: newValue
                                    });
                                    doFilter(target);
                                }
                                else if (newValue.length == 36) {
                                    var val = $(this).combobox('getText');
                                    addFilterRule(target, {
                                        field: $(this).attr('name'),
                                        op: 'equal',
                    ...
                    else if (input.hasClass('numberbox-f')) { //数值框
                        input.textbox('textbox').bind('keydown', function (e) {
                            if (e.keyCode == 13) {
                                var inputDom = $(this).parents('div.datagrid-filter-c').find('input.datagrid-filter');
                                addFilterRule(target, {
                                    field: inputDom.attr('name'),
                                    op: 'equal',
                                    value: $(this).val()
                                });
                                doFilter(target);
                    ...
                    else if (input.hasClass('dt')) { //日期框
                        $(input.find('.dt')).change(function () {
                            var val = $(this).val();
                            if (val) {
                                var fieldName = $(this).parent().attr('name');
                                var token = val.split('至');
                                addFilterRule(target, {
                                    field: fieldName,
                                    op: 'greater',
                                    value: token[0] + '00:00:00'
                                });
                                addFilterRule(target, {
                                    field: fieldName + '_End',
                                    op: 'less',
                                    value: token[1] + ' 23:59:59'
                                });
                                doFilter(target);
                                $(this).val(val);
                            }
...
$.extend($.fn.datagrid.defaults.filters, {
    dateRange: {
        init: function (container, options) {
            var c = $('<div style="display:inline-block" class="dt"><input type="text" class="dt" /></div>').appendTo(container);
            c.find('.dt').dateranges();
            return c;
        },
        destroy: function (target) {
            $(target).find('.dt').remove();
        },
        getValue: function (target) {
            return $(target).find('.dt').val();
        },
        setValue: function (target, value) {
            $(target).find('.dt').val(value);
        },
        resize: function (target, width) {
            $(target)._outerWidth(width)._outerHeight(22);
            $(target).find('.dt').width(width);
        }
    }
});