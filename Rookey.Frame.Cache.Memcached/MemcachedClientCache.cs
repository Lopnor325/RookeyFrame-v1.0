...
namespace Rookey.Frame.Cache.Memcached
{
    ...
    public class MemcachedClientCache
        : ServiceStack.Caching.ICacheClient, ServiceStack.Caching.IMemcachedClient
    {
        ...
        {
            var ipEndpoints = new List<IPEndPoint>() { new IPEndPoint(IPAddress.Parse("127.0.0.1"), 11211) };
        ...
        public MemcachedClientCache(IEnumerable<string> hosts, int minPoolSize, int maxPoolSize, int connectionTimeout, int deadTimeout)
            : this(hosts)
        {
            this._minPoolSize = minPoolSize > 0 ? minPoolSize : 10;
            this._maxPoolSize = maxPoolSize > 0 ? maxPoolSize : 100;
            this._connectionTimeout = connectionTimeout > 0 ? connectionTimeout : 10;
            this._deadTimeout = deadTimeout > 0 ? deadTimeout : 2;
        }
        ...
        private IMemcachedClientConfiguration PrepareMemcachedClientConfiguration(IEnumerable<IPEndPoint> ipEndpoints)
        {
            ...
            config.SocketPool.MinPoolSize = _minPoolSize > 0 ? _minPoolSize : 10;
            config.SocketPool.MaxPoolSize = _maxPoolSize > 0 ? _maxPoolSize : 100;
            config.SocketPool.ConnectionTimeout = new TimeSpan(0, 0, _connectionTimeout > 0 ? _connectionTimeout : 10);
            config.SocketPool.DeadTimeout = new TimeSpan(0, _deadTimeout > 0 ? _deadTimeout : 2, 0);
